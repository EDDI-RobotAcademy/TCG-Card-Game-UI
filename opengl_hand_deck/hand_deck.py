...
class HandDeck:
    ...
    def redraw(self):
        ...
        for hand_deck in self.battle_field.get_pickable_hand_deck_base():
            get_pickable_hand_deck_base = hand_deck.get_pickable_hand_deck_base()
            get_pickable_hand_deck_base.draw()

            attached_shape_list = get_pickable_hand_deck_base.get_attached_shapes()
    ...
    def on_canvas_drag(self, event):
        ...
        if self.selected_object and self.drag_start:
            ...
            print(f"pickable_unit: {pickable_hand_deck}")
    ...
    def on_canvas_click(self, event):
        ...
        try:
            ...
            for hand_deck in self.battle_field.get_pickable_hand_deck_base():
                print(f"type(hand_deck): {type(hand_deck)}")
                if isinstance(hand_deck, HandDeck):
                    hand_deck.selected = False

            self.selected_object = None

            for hand_deck in reversed(self.battle_field.get_pickable_hand_deck_base()):
                print("find selected Pickable Rectangle")
                pickable_hand_deck_base = hand_deck.get_pickable_hand_deck_base()
