...
class LegacyEnergyCard():
    ...
    def create_energy_race_illustration_circle(self, image_data, center, radius):
        energy_race_circle = CircleImage(image_data=image_data,
                                         center=center,
                                         radius=radius)
        self.add_shape(energy_race_circle)

    def create_energy_type_illustration_circle(self, image_data, center, radius):
        energy_type_circle = CircleImage(image_data=image_data,
                                         center=center,
                                         radius=radius)
        self.add_shape(energy_type_circle)


    def init_shapes(self, circle_radius, card_number, rectangle_height, rectangle_width):

        self.create_energy_race_illustration_circle(
