...
from card_shop_frame.service.request.my_game_money_request import MyGameMoneyRequest
...
class CardShopMenuFrameServiceImpl(CardShopMenuFrameService):
    ...
    def createCardShopUiFrame(self, rootWindow, switchFrameWithMenuName):
        ...
        # MyGameMoneyresponseData = self.__cardShopMenuFrameRepository.requestMyGameMoney(
        #     MyGameMoneyRequest(self.__sessionRepository.get_session_info))
        # print(MyGameMoneyresponseData)
        ...
        get_new_all_cards_button = tkinter.Button(cardShopMenuFrame, text="전체 카드 뽑기", bg="#2E2BE2", fg="white",
                                                command=lambda: (switchFrameWithMenuName("buy-random-card"),
                                                                 BuyRandomCardRequest.setRaceInfo(self.__sessionRepository.get_session_info,
                                                                                                  "전체")), width=36,
                                                height=4)
