...
class PreDrawedBattleFieldFrameRefactor(OpenGLFrame):
    ...
    def on_canvas_release(self, event):
        print("detect mouse release")

        x, y = event.x, event.y
        y = self.winfo_reqheight() - y

        if self.selected_object and self.drag_start:
            print("on_canvas_release detect selected object ?")
            pickable_card = self.selected_object.get_pickable_card_base()
            y *= -1
    ...
    def point_inside_polygon(self, x, y, poly):
        print(f"check point_inside_polygon - x: {x}, y: {y}")
        n = len(poly)
        inside = False
...
class TestDrawBattleFieldHandToFieldDetection(unittest.TestCase):

    def test_draw_battle_field_hand_to_field_detection(self):
